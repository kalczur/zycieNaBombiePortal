...
const Products = ({ match }) => {
  ...
  return (
    <div className="products">
      <h1 className="component-header">Produkty</h1>
      <div className="products__list">
        {Products.map(({ name, weight, price, brand }) => {
          if (fixBrandName(brand) === match.params.brand)
            return <Product key={name} name={name} weight={weight} price={price} brand={brand}></Product>;
        })}
      </div>
    </div>
  );
